  ...
  stages {
    ...
    stage('Build webserver image') {
      ...
      steps {
        ...
        sh "ssh ubuntu@${ipadr.buildserver_ip.value} sudo docker build --tag webserver:latest /webserver/"
        ...
        sh "ssh ubuntu@${ipadr.buildserver_ip.value} sudo docker tag webserver:latest ${env.DOCKERHUB_CREDS_USR}/webserver:3"
        sh "ssh ubuntu@${ipadr.buildserver_ip.value} sudo docker push ${env.DOCKERHUB_CREDS_USR}/webserver:3"
